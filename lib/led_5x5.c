...
// ARQUIVO .pio
#include "build/semaforo_inteligente_freertos.pio.h"
...
void setup_PIO() {
    PIO pio = pio0;
    uint offset = pio_add_program(pio, &semaforo_inteligente_freertos_program);
    uint sm = pio_claim_unused_sm(pio, true);
    semaforo_inteligente_freertos_program_init(pio, sm, offset, OUT_PIN);
